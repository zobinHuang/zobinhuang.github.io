...
async function _load_code_segment(){
    ...
    function __process_language_name(p_language){
        ...
        else if(p_language == 'cuda') return 'CUDA'
        else if(p_language == 'ptx') return 'PTX'
        else if(p_language == 'sass') return 'SASS'
    ...
    for(let i = 0; i < code_segments.length; i++){
        ...
        // 获取代码段语言
        var code_segment_language = null
        if(code_segments[i].classList[1] === "plaintext")
            code_segment_language = __process_language_name(code_segment_meta.getAttribute("code"))
        else
            code_segment_language = __process_language_name(`${code_segments[i].classList[1]}`)
